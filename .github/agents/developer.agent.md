...
2. If a Design Spec or Design Addendum is present, read it fully before writing any code:
   ...
   - For every screen listed in the Stitch Project Reference, call `mcp_stitch_get_screen` **before writing any code for that screen**. Use the mockup as the authoritative visual source of truth for layout, component placement, and UI states. Do not implement visual layout from prose alone when a Stitch screen ID is available.
